...
<tr>
	<td class = "jhg">
		...
		<form action = "stuClasses.php" method = "post">
		...
		if(isset($_POST['find']))
		{
			...
			{
				$name = "SELECT name FROM Student WHERE userId = " . $userId;
				...
				$q = "SELECT C.classId, C.classNum, C.className FROM Class as C WHERE C.classId IN (SELECT t1.classId FROM Takes AS t1 WHERE userId = " . $userId . ")";
